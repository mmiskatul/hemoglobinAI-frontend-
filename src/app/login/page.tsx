...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "verify") {
        ...
        window.location.href = "/requester"; return;
      }
      if (mode === "forgot") {
  ...
  return <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6"><section className="w-full max-w-md bg-white border border-slate-200 rounded-3xl shadow-xl p-8">
    ...
    <form onSubmit={submit} className="mt-6 space-y-4">
      ...
      {!["verify", "forgot"].includes(mode) && <input required minLength={8} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password (8+ characters)" className="w-full rounded-xl border border-slate-300 px-4 py-3" />}

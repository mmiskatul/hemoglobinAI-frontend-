...
export default function LoginPage() {
  ...
  return <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6"><section className="w-full max-w-md bg-white border border-slate-200 rounded-3xl shadow-xl p-8">
    ...
    <form onSubmit={submit} className="mt-6 space-y-4">
      ...
      {mode === "register" && <select value={role} onChange={e => setRole(e.target.value)} className="w-full rounded-xl border border-slate-300 px-4 py-3"><option value="requester">Requester</option><option value="donor">Donor</option><option value="hospital">Hospital</option><option value="agent">Coordination Agent</option></select>}
